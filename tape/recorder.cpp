...
#include "object.h"
#include "aggregate.h"
#include "gld_complex.h" // add this near the top, before using 'complex', Re(), Im()
...
static int recorder_open(OBJECT *obj)
{
	...
	if (my->type == FT_FILE && my->multifile[0] != 0)
	{
		...
		else
		{
			...
			const char *host = std::getenv("COMPUTERNAME"); // not MACHINENAME
#else
