...
#endif
		oclass = gld_class::create(mod, const_cast<char *>("group_recorder"), sizeof(group_recorder), PC_POSTTOPDOWN);
		if (oclass == nullptr)
	...
	try
	{
		*obj = gl_create_object(group_recorder::oclass);
		if (*obj != nullptr)
		{
			...
			if (!my) {
				gl_error("create_group_recorder: obj->data is null for class 'group_recorder'");
				return 0;
			}
...
#else
extern "C" MODULE_API TIMESTAMP sync_group_recorder(void *object, ...)
{

	// Add early validation of callback
    if (!callback) {
        gl_error("sync_group_recorder: callback is null");
        return TS_INVALID;
    }

    if (!callback->time.local_datetime) {
        gl_error("sync_group_recorder: local_datetime function is null");
        return TS_INVALID;
    }
