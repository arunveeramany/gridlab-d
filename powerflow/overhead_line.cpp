...
overhead_line::overhead_line(MODULE *mod) : line(mod)
{
	if(oclass == nullptr)
	{
		...
		oclass = gld_class::create(mod,"overhead_line",sizeof(overhead_line),PC_PRETOPDOWN|PC_BOTTOMUP|PC_POSTTOPDOWN|PC_UNSAFE_OVERRIDE_OMIT|PC_AUTOLOCK);
		if (oclass==nullptr)
...
//{
extern "C" TIMESTAMP sync_overhead_line(void *object, ...)
{
