...
fuse::fuse(MODULE *mod) : link_object(mod)
{
	if(oclass == nullptr)
	{
		...
		oclass = gld_class::create(mod,"fuse",sizeof(fuse),PC_PRETOPDOWN|PC_BOTTOMUP|PC_POSTTOPDOWN|PC_UNSAFE_OVERRIDE_OMIT|PC_AUTOLOCK);
		if (oclass==nullptr)
...
extern "C" MODULE_API TIMESTAMP sync_fuse(void *object, ...)
{
	...
	if (!callback) {
        gl_error("callback is null in sync_fuse");
