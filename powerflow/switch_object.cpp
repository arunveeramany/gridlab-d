...
switch_object::switch_object(MODULE *mod) : link_object(mod)
{
	if(oclass == nullptr)
	{
		...
		oclass = gld_class::create(mod,"switch",sizeof(switch_object),PC_PRETOPDOWN|PC_BOTTOMUP|PC_POSTTOPDOWN|PC_UNSAFE_OVERRIDE_OMIT|PC_AUTOLOCK);
		if (oclass== nullptr)
...
extern "C" TIMESTAMP sync_switch(void *object, ...)
{
    va_list args;
