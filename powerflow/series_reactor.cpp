...
series_reactor::series_reactor(MODULE *mod) : link_object(mod)
{
	if(oclass == nullptr)
	{
		...
		oclass = gld_class::create(mod,"series_reactor",sizeof(series_reactor),PC_PRETOPDOWN|PC_BOTTOMUP|PC_POSTTOPDOWN|PC_UNSAFE_OVERRIDE_OMIT|PC_AUTOLOCK);
		if (oclass==nullptr)
...
extern "C" MODULE_API TIMESTAMP sync_series_reactor(void *object, ...)
{
    ...
    if (!callback) {
        gl_error("sync_series_reactor: callback is null");
        return TS_INVALID;
    }

    if (!callback->time.local_datetime) {
        gl_error("sync_series_reactor: local_datetime function is null");
        return TS_INVALID;
    }
