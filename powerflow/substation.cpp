...
substation::substation(MODULE *mod) : node(mod)
{
	...
	if (oclass==nullptr)
	{
		...
		oclass = gld_class::create(mod,"substation",sizeof(substation),PC_PRETOPDOWN|PC_BOTTOMUP|PC_POSTTOPDOWN|PC_UNSAFE_OVERRIDE_OMIT|PC_AUTOLOCK);
		if (oclass==nullptr)
...
#else
extern "C" MODULE_API TIMESTAMP sync_substation(void *object, ...)
{
    va_list args;
	...
	if (!callback) {
        gl_error("callback is null in sync_substation");
