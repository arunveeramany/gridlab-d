...
sync_check::sync_check(MODULE *mod) : powerflow_object(mod)
{
	if (oclass == nullptr)
	{
		...
		oclass = gld_class::create(mod, "sync_check", sizeof(sync_check), PC_PRETOPDOWN | PC_BOTTOMUP | PC_POSTTOPDOWN | PC_AUTOLOCK);
		if (oclass == nullptr)
...
extern "C" MODULE_API TIMESTAMP sync_sync_check(void *object, ...)
{
	...
	if (!callback) {
        gl_error("callback is null in sync_sync_check");
