...
regulator::regulator(MODULE* mod) : link_object(mod)
{
    if (oclass == nullptr)
    {
        ...
        if (gl_publish_function(oclass, "check_limits_pwr_object",
            ...
            GL_THROW(
                "Unable to publish regulator external power limit calculation "
                "function");
    ...
    */
    ...
    switch (pConfig->connect_type)
    {
        ...
        case regulator_configuration::OPEN_DELTA_ABBC:
            ...
            gl_warning(
                "Only WYE-WYE configurations are working in either "
    ...
    else if (iteration_flag)
    {
        if (pConfig->control_level == pConfig->INDIVIDUAL)
        {
            ...
            if (pConfig->connect_type == pConfig->WYE_WYE)
            {
                ...
                GL_THROW(
                    "Specified connect type is not supported in automatic modes "
                    "at this time.");
            /* TROUBLESHOOT
        ...
        if (prev_time == curr_time_value)
        {
            ...
            if (tap_A_changed == 1)
            {
                if (initial_tap_A == tap[0])
                {
                    ...
                    {
                        gl_error(
                            "Unusual control of the regulator has resulted in a "
