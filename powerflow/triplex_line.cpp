...
triplex_line::triplex_line(MODULE *mod) : line(mod)
{
	if(oclass == nullptr)
	{
		...
		oclass = gld_class::create(mod,"triplex_line",sizeof(triplex_line),PC_PRETOPDOWN|PC_BOTTOMUP|PC_POSTTOPDOWN|PC_UNSAFE_OVERRIDE_OMIT|PC_AUTOLOCK);
		if (oclass==nullptr)
...
extern "C" TIMESTAMP sync_triplex_line(void *object, ...)
{

	// Add early validation of callback
    if (!callback) {
        gl_error("sync_triplex_line: callback is null");
        return TS_INVALID;
    }

    if (!callback->time.local_datetime) {
        gl_error("sync_triplex_line: local_datetime function is null");
        return TS_INVALID;
    }
