...
line::line(MODULE *mod) : link_object(mod) {
	if(oclass == nullptr)
	{
		...
		line_class = oclass = gld_class::create(mod,"line",sizeof(line),PC_PRETOPDOWN|PC_BOTTOMUP|PC_POSTTOPDOWN|PC_UNSAFE_OVERRIDE_OMIT|PC_AUTOLOCK);
		if (oclass==nullptr)
...
extern "C" TIMESTAMP sync_line(void *object, ...)
{
    ...
    if (!callback) {
        gl_error("sync_line: callback is null");
        return TS_INVALID;
    }

    if (!callback->time.local_datetime) {
        gl_error("sync_line: local_datetime function is null");
        return TS_INVALID;
    }
