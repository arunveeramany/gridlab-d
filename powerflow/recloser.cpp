...
recloser::recloser(MODULE *mod) : switch_object(mod)
{
	if(oclass == nullptr)
	{
		...
		oclass = gld_class::create(mod,"recloser",sizeof(recloser),PC_PRETOPDOWN|PC_BOTTOMUP|PC_POSTTOPDOWN|PC_UNSAFE_OVERRIDE_OMIT|PC_AUTOLOCK);
        if(oclass == nullptr)
...
extern "C" TIMESTAMP sync_recloser(void *object, ...)
{
	...
	if (!callback) {
        gl_error("callback is null in sync_recloser");
