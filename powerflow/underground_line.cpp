...
underground_line::underground_line(MODULE *mod) : line(mod)
{
	if(oclass == nullptr)
	{
		...
		oclass = gld_class::create(mod,"underground_line",sizeof(underground_line),PC_PRETOPDOWN|PC_BOTTOMUP|PC_POSTTOPDOWN|PC_UNSAFE_OVERRIDE_OMIT|PC_AUTOLOCK);
		if (oclass==nullptr)
...
#else
extern "C" MODULE_API TIMESTAMP sync_underground_line(void *object, ...)
{
    va_list args;
