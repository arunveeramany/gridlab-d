...
motor::motor(MODULE *mod):node(mod)
{
	if(oclass == nullptr)
	{
		...
		oclass = gld_class::create(mod,"motor",sizeof(motor),passconfig|PC_UNSAFE_OVERRIDE_OMIT|PC_AUTOLOCK);
		if (oclass==nullptr)
...
#else
extern "C" MODULE_API TIMESTAMP sync_motor(void *object, ...)
{
	// Add early validation of callback
    if (!callback) {
        gl_error("sync_motor: callback is null");
        return TS_INVALID;
    }
    ...
    if (!callback->time.local_datetime) {
        gl_error("sync_motor: local_datetime function is null");
        return TS_INVALID;
    }
