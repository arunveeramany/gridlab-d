...
triplex_meter::triplex_meter(MODULE *mod) : triplex_node(mod)
{
	...
	if (oclass == nullptr)
	{
		...
		oclass = gld_class::create(mod, "triplex_meter", sizeof(triplex_meter), PC_PRETOPDOWN | PC_BOTTOMUP | PC_POSTTOPDOWN | PC_UNSAFE_OVERRIDE_OMIT | PC_AUTOLOCK);
		if (oclass == nullptr)
...
TIMESTAMP triplex_meter::postsync(TIMESTAMP t0, TIMESTAMP t1)
{
	...
	measured_real_power = (indiv_measured_power[0]).Re() + (indiv_measured_power[1]).Re();
	+(indiv_measured_power[2]).Re();
