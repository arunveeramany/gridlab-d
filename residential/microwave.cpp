...
void microwave::init_noshape()
{
    ...
    {
        gl_warning(
            "microwave installed power is smaller than traditional "
            "microwave ovens");
    }
    else if (shape.params.analog.power > 1.800)
    {
        gl_warning(
            "microwave installed power is greater than traditional "
            "microwave ovens");
    }
    if (standby_power < 0)
    {
    ...
    {
        gl_warning(
            "standby power exceeds installed power, resetting to 1%% of "
            "installed power");
        standby_power = shape.params.analog.power * 0.01;
    }
    if (cycle_time < 0)
...
int microwave::init(OBJECT* parent)
{
    ...
    {
        gl_error(
            "queued loadshapes not supported ~ will attempt to run as an "
            "unshaped load");
