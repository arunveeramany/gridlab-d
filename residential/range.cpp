...
int range::init(OBJECT* parent)
{
    ...
    if (oven_setpoint < 90 || oven_setpoint > 160)
        GL_THROW(
            "This model is experimental and not validated: oven thermostat is "
    ...
    */
    ...
    if (thermostat_deadband > 10 || thermostat_deadband < 0.0)
        GL_THROW(
            "oven deadband of %f is outside accepted bounds of 0 to 10 "
            "degrees (5.6 degC).",
            thermostat_deadband);
    ...
    switch (shape.type)
    {
        case MT_UNKNOWN:
            ...
            gl_warning(
                "This device, %s, is considered very experimental and has not "
                "been validated.",
                get_name());
            break;
        case MT_ANALOG:
        ...
        case MT_MODULATED:
            ...
            {
                GL_THROW(
                    "Amplitude modulated oven usage is nonsensical for residential "
                    "ovens");
                /*	TROUBLESHOOT
...
TIMESTAMP range::presync(TIMESTAMP t0, TIMESTAMP t1)
{
    ...
    if (Tw > 212.0)
    {
        ...
        gl_warning(
            "range:%i is using an experimental model and should not be "
            "considered reliable",
            my->id);
        /*	TROUBLESHOOT
...
double range::actual_kW(void)
{
    ...
    if (heat_needed && re_override != OV_OFF)
    {
        ...
        if (actual_voltage > 2.0 * default_line_voltage)
        {
            if (trip_counter++ > 10)
                GL_THROW(
                    "oven line voltage for range:%d is too high, exceeds twice "
                    "nominal voltage.",
                    obj->id);
            /*	TROUBLESHOOT
