...
ZIPload::ZIPload(MODULE *module) : residential_enduse(module)
{
	...
	if (oclass == nullptr)
	{
		...
		oclass = gld_class::create(module, "ZIPload", sizeof(ZIPload), PC_BOTTOMUP | PC_AUTOLOCK);
		if (oclass == nullptr)
		...
		if (gl_publish_variable(oclass,
								...
								PT_enumeration, "configuration", PADDR(configuration),
								PT_KEYWORD, "IS110", (enumeration)ZIPCONF_IS110,
								PT_KEYWORD, "IS220", (enumeration)ZIPCONF_IS220,
								PT_DESCRIPTION, "Load wiring: IS110 = line-to-neutral (~120 V), IS220 = line-to-line (~240 V)",
...
int ZIPload::create()
{
	...
	configuration = ZIPCONF_UNSPECIFIED; // do not override is_240 unless user sets it
	is_240 = false;						 // existing default
...
int ZIPload::init(OBJECT *parent)
{
	...
	// Heat-only mode first: zero electrical fractions, mark HEATLOAD
	if (heatgain_only == true)
	{
		load.config = EUC_HEATLOAD;
		load.power_fraction = load.current_fraction = load.impedance_fraction = 0.0;
	...
	// Reconcile 'configuration' vs 'is_240' BEFORE setting EUC_IS220
	if (configuration != ZIPCONF_UNSPECIFIED)
	{
		bool config_is_240 = (configuration == ZIPCONF_IS220);
		if (is_240 != config_is_240)
		{
			gl_warning("ZIPload::init: 'configuration' (%s) overrides 'is_240' (%s).",
					   config_is_240 ? "IS220" : "IS110", is_240 ? "TRUE" : "FALSE");
		}
		is_240 = config_is_240;
	}

	// Apply the two-phase bit explicitly (set or clear)
	if (is_240)
	{
		load.config |= EUC_IS220; // set two-phase (line-to-line ~240 V)
	}
	else
	{
		load.config &= ~EUC_IS220; // clear two-phase (line-to-neutral ~120 V)
	}
