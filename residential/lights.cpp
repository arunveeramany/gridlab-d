...
int lights::init(OBJECT* parent)
{
    ...
    if (load.power_factor == 0)
    {
        ...
        gl_warning(
            "No value was given for power factor. Defaulting to power "
    ...
    if (load.voltage_factor == 0)
    {
        load.voltage_factor = 1.0;
        gl_warning(
            "No value was given for voltage factor.  Defaulting to voltage "
            "factor 1.00");
    }
    ...
    if (shape.params.analog.power == 0 && shape.schedule == nullptr)
    {
        ...
        {
            gl_error(
                "lights parent must publish \'floor_area\' to work properly if "
...
TIMESTAMP lights::sync(TIMESTAMP t0, TIMESTAMP t1)
{
    ...
    { /* manual power calculation*/
        ...
        {
            gl_warning(
                "lights shape demand exceeds installed lighting power, "
                "capping to 100%%");
            shape.load = 1.0;
        }
