...
int evcharger_det::init(OBJECT* parent)
{
    ...
    if (CarInformation.MaxChargeRate > 1700)
    {
        ...
        {
            gl_warning(
                "evcharger_det:%s - The max charge rate is over 1.7 kW (Level "
    ...
    {
        gl_warning(
            "evcharger_det:%s - the breaker rating may be too low for this "
            "maximum charge rate.",
            hdr->name ? hdr->name : "unnamed");
        /*  TROUBLESHOOT
    ...
    {
        GL_THROW(
            "evcharger_det:%d %s - maximum_overload_current must be positive "
            "and non-zero!",
    ...
    if (CarInformation.battery_size <= 0.0)
    {
        ...
        {
            GL_THROW(
                "Battery size is not specified, nor are the mileage "
    ...
    if (deltamode_inclusive)
    {
        ...
        {
            gl_warning(
                "evcharger_det:%s indicates it wants to run deltamode, but "
    ...
    else  // Not enabled for this model
    {
        ...
        {
            gl_warning(
                "evcharger_det:%d %s - Deltamode is enabled for the module, "
                "but not this house!",
                hdr->id, (hdr->name ? hdr->name : "Unnamed"));
            /*  TROUBLESHOOT
