...
int waterheater::init(OBJECT* parent)
{
    ...
    if (tank_volume <= 0.0)
    {
        if (tank_diameter <= 0)
        {
            if (tank_height <= 0)
            {
                ...
                gl_warning(
                    "waterheater::init() : tank volume, diameter, and height "
            ...
            else
            {
                ...
                gl_warning(
                    "waterheater::init() : tank volume and diameter were not "
        ...
        else
        {
            if (tank_height <= 0)
            {
                ...
                gl_warning(
                    "waterheater::init() : tank volume and height were not "
                    "specified, defaulting to 50 gallons");
    ...
    if (tank_setpoint < 90 || tank_setpoint > 160)
        gl_error(
            "watertank thermostat is set to %f and is outside the bounds of "
    ...
    */
    ...
    if (thermostat_deadband > 10 || thermostat_deadband < 0.0)
        GL_THROW(
            "watertank deadband of %f is outside accepted bounds of 0 to 10 "
            "degrees (5.6 degC).",
            thermostat_deadband);
    if (current_model != FORTRAN)
    {
    ...
    if (current_model == FORTRAN)
    {
        ...
        {
            GL_THROW(
                "The simulation time for the fortran water heater model must be "
                "greater than 0.");
        }
    ...
    if (current_model == MULTILAYER)
    {
        ...
        {
            gl_warning(
                "waterheater::init() : Mixing valve temperature was not set, "
        ...
        {
            gl_warning(
                "waterheater::init() : Mixing valve temperature is set very "
...
void waterheater::thermostat(TIMESTAMP t0, TIMESTAMP t1)
{
    ...
    switch (tank_status)
    {
        ...
        default:
            GL_THROW(
                "waterheater thermostat() detected that the water heater tank is "
                "in an unknown state");
    }
...
TIMESTAMP waterheater::sync(TIMESTAMP t0, TIMESTAMP t1)
{
    ...
    {
        if (t1 == fwh_sim_time)
        {  // update power, cop, water temperature, and heat
...
double waterheater::actual_kW(void)
{
    ...
    if (heat_needed && re_override != OV_OFF)
    {
        ...
        if (actual_voltage > 2.0 * nominal_voltage)
        {
            if (trip_counter++ > 10)
                gl_error(
                    "Water heater line voltage for waterheater:%d is too high, "
