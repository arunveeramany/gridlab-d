	...
	try
	{
		*obj = gl_create_object(supervisory_control::oclass);
		if (*obj!=nullptr)
		{
			...
			if (!my) {
				gl_error("create_supervisory_control: obj->data is null for class 'supervisory_control'");
				return 0;
			}
...
#else
extern "C" MODULE_API TIMESTAMP sync_supervisory_control(void *object, ...)
{
    va_list args;
    ...
    if (!callback) {
        gl_error("callback is null in sync_supervisory_control");
