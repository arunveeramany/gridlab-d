...
stub_bidder::stub_bidder(MODULE *module)
{
	if (oclass==nullptr)
	{
		oclass = gld_class::create(module,"stub_bidder",sizeof(stub_bidder),PC_BOTTOMUP|PC_AUTOLOCK);
		if (oclass==nullptr)
	...
	try
	{
		*obj = gl_create_object(stub_bidder::oclass);
		if (*obj!=nullptr)
		{
			...
			if (!my) {
				gl_error("create_stub_bidder: obj->data is null for class 'stub_bidder'");
				return 0;
			}
...
#else
extern "C" MODULE_API TIMESTAMP sync_stub_bidder(void *object, ...)
{
    va_list args;
    ...
    if (!callback) {
        gl_error("callback is null in sync_stub_bidder");
