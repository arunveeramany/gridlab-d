...
stubauction::stubauction(MODULE *module)
{
	if (oclass == nullptr)
	{
		oclass = gld_class::create(module, "stubauction", sizeof(stubauction), passconfig | PC_AUTOLOCK);
		if (oclass == nullptr)
...
#else
extern "C" MODULE_API TIMESTAMP sync_stubauction(void *object, ...)
{
    va_list args;
    ...
    if (!callback) {
        gl_error("callback is null in sync_stubauction");
