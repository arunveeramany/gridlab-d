...
{
	if(oclass == nullptr){
		oclass = gld_class::create(mod,"passive_controller",sizeof(passive_controller),PC_PRETOPDOWN|PC_BOTTOMUP|PC_POSTTOPDOWN|PC_AUTOLOCK);
		if (oclass==nullptr)
...
#else
extern "C" MODULE_API TIMESTAMP sync_passive_controller(void *object, ...)
{
    va_list args;
    ...
    if (!callback) {
        gl_error("callback is null in sync_passive_controller");
