...
double_assert::double_assert(MODULE *module)
{
	if (oclass == nullptr)
	{
		...
		if (gl_publish_variable(oclass,
								...
								PT_bool, "skip_if_zero", get_skip_if_zero_offset(), PT_DESCRIPTION, "Skip assert if measured value is zero",
								nullptr) < 1)
		{
			char msg[256];
...
TIMESTAMP double_assert::commit(TIMESTAMP t1, TIMESTAMP t2)
{
	...
	// Check if parent object is out of service or has flags indicating isolation
	if (parent != nullptr)
	{
		// Check the object's flags for out-of-service condition
		if ((parent->flags & OF_DEFERRED) != 0)
		{
			gl_verbose("Assert skipped on %s - parent is deferred",
					   parent->name);
			return TS_NEVER;
		}
	}
	...
	gld_property target_prop(parent->name, get_target().c_str());
	...
	if (skip_if_zero && fabs(x) < 1e-12)
	{
		gl_verbose("Assert skipped on %s - target value is zero (possibly isolated)",
				   parent->name);
		return TS_NEVER;
	}
	...
	if (status == ASSERT_TRUE)
	{
		...
		// Skip if actual value is NaN or zero (indicating inactive/isolated)
		if (_isnan(x) || (value != 0.0 && fabs(x) < 1e-12))
		{
			gl_verbose("Assert skipped on %s - target appears inactive",
					   parent->name);
			return TS_NEVER;
		}
		...
		if (_isnan(m) || m > range)
		{
			gl_error("Assert failed on %s: %s %g not within %f of given value %g",
					 parent->name, get_target().c_str(), x, range, value);

			// Log expected failures for error tests
			if (is_error_test)
			{
				gl_verbose("Expected failure in error test object %s", parent->name);
			}
			return TS_INVALID; // Changed from 0 to TS_INVALID
		}
		gl_verbose("Assert passed on %s", parent->name);
		return TS_NEVER;
	}
	...
	{
		gl_verbose("Assert test is not being run on %s", parent->name);
		return TS_NEVER;
	}
}
...
EXPORT SIMULATIONMODE update_double_assert(OBJECT *obj, TIMESTAMP t0, unsigned int64 delta_time, unsigned long dt, unsigned int iteration_count_val)
{
	...
	if (iteration_count_val == 0)
	{
		...
		if (delta_time >= dt)
		{
			...
			if (x == nullptr)
			{
				gl_error("Specified target %s for %s is not valid.", da->get_target().c_str(), gl_name(obj->parent, buff, 59));
				/*  TROUBLESHOOT
