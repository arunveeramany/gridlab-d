...
int sec_control::init(OBJECT* parent)
{
    ...
    if (deltamode_inclusive)
    {
        ...
        {
            gl_warning(
                "sec_control:%s indicates it wants to run deltamode, but the "
        ...
        else
        {
            ...
            {
                GL_THROW(
                    "sec_control:%s - failed to add object to generator deltamode "
                    "object list",
                    obj->name ? obj->name : "unnamed");
                /*  TROUBLESHOOT
    ...
    else  // This particular model isn't enabled
    {
        ...
        {
            gl_warning(
                "sec_control:%d %s - Deltamode is enabled for the module, but "
                "not this controller!",
...
void sec_control::participant_tlp_check(SEC_CNTRL_PARTICIPANT* obj)
{
    if (curr_dt > 0)  // in delta mode therefore deltat is present
    {
        ...
        {
            gl_warning(
                "sec_control: participant %s: Tlp = %0.5f < dt = %0.5f. "
...
void sec_control::update_pdisp(SEC_CNTRL_PARTICIPANT& obj, double val)
{
    if (val != 0)
    {
        ...
        {
            gl_warning(
                "sec_control: Object %s desired setpoint change of %0.6f MW, "
        ...
        {
            gl_warning(
                "sec_control: generator %s is dispatched at %0.4f pu. The "
...
void sec_control::get_perr(void)
{
    if (sampleflag_f)  // only update delta f if sampling
    {
        ...
        {
            gl_warning(
                "sec_control: overfrequency_limit (%0.3f Hz) is SMALLER than "
        ...
        {
            gl_warning(
                "sec_control: under (%0.3f Hz) is GREATER than nominal (%0.3f "
...
void sec_control::add_obj(std::vector<std::string>& vals)
{
    ...
    else if (gl_object_isa(tmp.ptr, "inverter_dyn", "generators"))
    {
        ...
        {
            GL_THROW(
                "sec_control: inverter_dyn control_mode %d is not supported. "
                "For object %s.",
    ...
    {
        GL_THROW(
            "sec_control: currently only diesel_dg, inverter_dyn and "
