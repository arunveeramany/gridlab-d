...
int inverter::init(OBJECT* parent)
{
    ...
    if (parent != nullptr)
    {
            ...
            gl_object_isa(parent, "triplex_load", "powerflow"))
        {
            ...
            if (deltamode_inclusive && (four_quadrant_control_mode == FQM_VSI))
            {
                ...
                if (parent->parent != nullptr)
                {
                    ...
                    {
                        GL_THROW(
                            "inverter:%s failed to map Norton-equivalence deltamode "
                            "variable from %s",
                    ...
                    else  // Implies it is a powerflow parent
                    {
                        ...
                        {
                            GL_THROW(
                                "diesel_dg:%s failed to map Norton-equivalence "
            ...
            if (deltamode_inclusive && (four_quadrant_control_mode == FQM_VSI))
            {
                ...
                {
                    GL_THROW(
                        "inverter:%s - Failed to map frequency checking variable "
                ...
                else  // Already populated, make sure it is the right size!
                {
                    ...
                    {
                        GL_THROW(
                            "inverter:%s exposed Norton-equivalent matrix is the "
                            "wrong size!",
                            obj->name ? obj->name : "unnamed");
                        /*  TROUBLESHOOT
                ...
                if (childed_connection == true)
                {
                    ...
                    {
                        GL_THROW(
                            "diesel_dg:%s failed to map Norton-equivalence deltamode "
                            "variable from %s",
                            obj->name ? obj->name : "unnamed",
                            parent->name ? parent->name : "unnamed");
                        // Defined above
                    ...
                    else  // Already populated, make sure it is the right size!
                    {
                        ...
                        {
                            GL_THROW(
                                "diesel_dg:%s exposed Norton-equivalent matrix is the "
                                "wrong size!",
                                obj->name ? obj->name : "unnamed");
                            // Defined above
                        }
            ...
            {
                GL_THROW(
                    "Unable to map phases property - ensure the parent is a meter "
                    "or triplex_meter");
                /*  TROUBLESHOOT
        ...
        {
            GL_THROW(
                "Inverter must have a valid powerflow object as its parent, or "
                "no parent at all");
            /*  TROUBLESHOOT
    ...
    {
        GL_THROW(
            "Inverter:%d %s - Inverter phases are incompatible with the "
        ...
        case FOUR_QUADRANT:
            ...
            {  // Volt_VAr init
                if (delay_time < 0)
                {
                    delay_time = 0.0;
                    gl_warning(
                        "Delay time for Volt/VAr mode unspecified or negative. "
                ...
                if (max_var_slew_rate <= 0)
                {
                    ...
                    gl_warning(
                        "Maximum VAr slew rate for Volt-VAr mode unspecified, "
                ...
                if (max_pwr_slew_rate <= 0)
                {
                    ...
                    gl_warning(
                        "Maximum output power slew rate for freq-power mode "
                ...
                if (VoltVArSchedInput.length() == 0)
                {
                    ...
                    gl_warning(
                        "Volt/VAr schedule unspecified. Setting inverter for "
                ...
                if (freq_pwrSchedInput.length() == 0)
                {
                    ...
                    gl_warning(
                        "Frequency-Power schedule unspecified. Setting power for "
                ...
                pf_reg == INCLUDED || four_quadrant_control_mode == FQM_GROUP_LF)
            {
                ...
                if (sense_object == nullptr)
                {
                    if (parent != nullptr)
                    {
                        ...
                        sense_object = parent;
                        gl_warning(
                            "inverter:%s - sense_object not specified for "
                    ...
                    {
                        gl_error(
                            "inverter:%s - LOAD_FOLLOWING and power-factor regulation "
                ...
                if (gl_object_isa(sense_object, "node", "powerflow"))
                {
                        ...
                        gl_object_isa(sense_object, "triplex_meter", "powerflow"))
                    {
                        ...
                        {
                            gl_error(
                                "inverter:%s - an error occurred while mapping the "
                        ...
                        {
                            gl_warning(
                                "inverter:%s is LOAD_FOLLOWING and/or power-factor "
                    ...
                    {
                        gl_error(
                            "inverter:%s - sense_object is a node, but not a meter or "
                            "triplex_meter!",
                            obj->name);
                        /*  TROUBLESHOOT
                ...
                else if (gl_object_isa(sense_object, "link", "powerflow"))
                {
                    ...
                    if (gl_object_isa(sense_object, "transformer", "powerflow"))
                    {
                        ...
                        {
                            gl_error(
                                "inverter:%s - inverter failed to map power calculation "
                ...
                if (pf_reg != EXCLUDED)
                {
                    if (pf_reg_activate_lockout_time < 0)
                    {
                        ...
                        gl_warning(
                            "inverter:%s - pf_reg_activate_lockout_time is "
                    ...
                    {
                        gl_warning(
                            "inverter:%s - pf_reg_activate_lockout_time is zero, "
                ...
                if (four_quadrant_control_mode == FQM_LOAD_FOLLOWING)
                {
                    ...
                    {
                        gl_warning(
                            "inverter:%s - charge_lockout_time is zero, oscillations "
                            "may occur",
                            obj->name);
                        /*  TROUBLESHOOT
                    ...
                    {
                        gl_warning(
                            "inverter:%s - discharge_lockout_time is zero, "
                            "oscillations may occur",
                            obj->name);
                        /*  TROUBLESHOOT
    ...
    if (use_multipoint_efficiency)
    {
        ...
        {  // all manufacturer defaults use the CEC
           // parameters
            case NONE:
                if (p_dco < 0)
                {
        ...
        {
            gl_error(
                "The maximum dc power into the inverter cannot be less than the "
    ...
    if (four_quadrant_control_mode == FQM_VOLT_VAR)
    {
        ...
        {
            gl_error(
                "inverter::init(): The curve was not constructed properly. V1 "
        ...
        {
            gl_error(
                "inverter::init(): The curve was not constructed properly. Q1 "
        ...
        {
            gl_warning(
                "volt var control lockout is 0. Warning this may cause "
    ...
    else if (four_quadrant_control_mode == FQM_VOLT_WATT)
    {
        ...
        {
            gl_error(
                "inverter::init(): The base voltage must be greater than 0 for "
                "VOLT_WATT.");
            return 0;
        }
        VW_m = (VW_P2 - VW_P1) /
        ...
        {
            gl_warning(
                "VOLT_WATT uses the VOLT_VAR lockout, which is 0. Warning "
    ...
    if (deltamode_inclusive)
    {
        ...
        {
            gl_warning(
                "inverter:%s indicates it wants to run deltamode, but the "
        ...
        else
        {
            ...
            {
                GL_THROW(
                    "inverter:%s - failed to add object to generator deltamode "
                    "object list",
                    obj->name ? obj->name : "unnamed");
                /*  TROUBLESHOOT
    ...
    else  // This particular model isn't enabled
    {
        ...
        {
            gl_warning(
                "inverter:%d %s - Deltamode is enabled for the module, but "
                "not this inverter!",
                obj->id, (obj->name ? obj->name : "Unnamed"));
            /*  TROUBLESHOOT
        ...
        else if (enable_1547_compliance)
        {
            ...
            gl_warning(
                "inverter:%d-%s - IEEE 1547 checks are enabled, but the model "
    ...
    else
    {
        if (pf_reg == INCLUDED)
        {
            if (t1 != t0)
            {
                ...
                if (pf_reg_activate == -2)
                {
                    ...
                    gl_warning(
                        "inverter:%s - pf_reg_activate undefined, setting to "
                ...
                if (pf_reg_deactivate == -1)
                {
                    ...
                    gl_warning(
                        "inverter:%s - pf_reg_deactivate undefined, setting to "
                ...
                {
                    gl_warning(
                        "inverter:%s - Very high values pf_reg_deactivate (~ "
                ...
                {
                    GL_THROW(
                        "inverter:%s - pf_reg_activate is greater than "
                        "pf_reg_deactivate.",
                        obj->name);
                }
                ...
                {
                    GL_THROW(
                        "inverter:%s - pf_reg_activate and/or pf_reg_deactivate are "
                        "negative.",
                        obj->name);
                }
                if (pf_reg_activate == pf_reg_deactivate)
        ...
        else if (pf_reg == INCLUDED_ALT)
        {
            if (t1 != t0)
            {
                ...
                if (pf_target_var == -2.0)
                {
                    pf_target_var = 0.95;
                    gl_warning(
                        "inverter:%s - pf_target_var is undefined, setting to a "
                        "default value of 0.95",
                        obj->name ? obj->name : "Unnamed");
                    /*  TROUBLESHOOT
                ...
                if (pf_reg_high == -2.0)
                {
                    pf_reg_high = -0.95;
                    gl_warning(
                        "inverter:%s - pf_reg_high is undefined, setting to a "
                        "default value of -0.95",
                        obj->name ? obj->name : "Unnamed");
                    /*  TROUBLESHOOT
                ...
                if (pf_reg_low == -2.0)
                {
                    pf_reg_low = 0.97;
                    gl_warning(
                        "inverter:%s - pf_reg_low is undefined, setting to a "
        ...
        if (four_quadrant_control_mode == FQM_LOAD_FOLLOWING)
        {
            if (t1 != t0)
            {
                ...
                {
                    GL_THROW(
                        "inverter:%s - charge_on_threshold is greater than "
                ...
                {
                    GL_THROW(
                        "inverter:%s - discharge_on_threshold is less than "
                ...
                {
                    gl_warning(
                        "inverter:%s - discharge_off_threshold should be larger "
        ...
        else if (four_quadrant_control_mode == FQM_GROUP_LF)
        {
            if (t1 != t0)
            {
                ...
                {
                    GL_THROW(
                        "inverter:%s - charge_threshold must be defined for "
                ...
                {
                    GL_THROW(
                        "inverter:%s - discharge_threshold must be defined for "
                        "GROUP_LOAD_FOLLOW mode.",
                        obj->name);
                }
                ...
                {
                    gl_warning(
                        "inverter:%s - charge_threshold and discharge_threshold "
                ...
                {
                    gl_error(
                        "inverter:%s - discharge_threshold must be larger than the "
                        "charge_threshold",
                        obj->name);
                }
            }  // t1 != t0
                                      ...
                                      // yet
        {
            ...
            if (first_iter_counter == 0)
            {
                ...
                if (four_quadrant_control_mode == FQM_CONSTANT_PQ)
                {
                    ...
                    {
                        GL_THROW(
                            "PQ_CONSTANT inverter:%s - failed to map additional "
                ...
                if (four_quadrant_control_mode == FQM_VSI)
                {
                    ...
                    if (obj->parent != nullptr)
                    {
                            ...
                            gl_object_isa(obj->parent, "elec_frequency", "powerflow"))
                        {
                            ...
                            {
                                GL_THROW(
                                    "Voltage source inverter:%s - failed to map "
        ...
        else  // FOUR_QUADRANT code
        {
            ...
            {  // == FQM_VOLT_VAR || == FQM_VOLT_WATT
                if (four_quadrant_control_mode ==
                    FQM_VOLT_VAR)
                {  // revalidating the parameters, we are in ::sync
                   // here, but maybe they changed?
                    ...
                    {
                        gl_error(
                            "inverter::sync(): The curve was not constructed "
                ...
                else if (four_quadrant_control_mode == FQM_VOLT_WATT)
                {
                    ...
                    {
                        gl_error(
                            "inverter::sync(): The base voltage must be greater than "
                            "0 for VOLT_WATT.");
                        return TS_INVALID;
                    }
            ...
            else if (four_quadrant_control_mode == FQM_CONSTANT_PQ)
            {
                ...
                if (b_soc != -1)
                {
                    ...
                    {
                        gl_warning(
                            "inverter:%s - battery at or below the SOC reserve - no "
                            "discharging allowed",
                            obj->name);
                    ...
                    if (fabs(temp_VA.Mag()) >
                        p_max)
                    {  // Requested power output (P_Out, Q_Out) is greater than
                       // inverter rating
                        if (p_max >
                ...
                {  // no battery is attached we are attached to a solar panel
                    ...
                    if (fabs(temp_VA.Mag()) >
                        VA_Efficiency)
                    {  // Requested power output (P_Out, Q_Out) is
            ...
            else if (four_quadrant_control_mode == FQM_VOLT_VAR_FREQ_PWR)
            {
                ...
                else
                {
                    ...
                    for (size_t i = 0; i < VoltVArSched.size();
                         i++)
                    {  // iterate over all specified voltage ranges, find where
            ...
            {
                gl_warning(
                    "inverter:%d - %s - Real power output (%g) exceeds maximum "
                    "DC output (%g)",
        ...
        lf_dispatch_change_allowed)
    {
                                          ...
                                          // we were doing
        {
            ...
            if (load_follow_status == DISCHARGE)
            {
                ...
                {
                    gl_verbose(
                        "inverter:%s - discharge desired, but not enough battery "
                        "capacity!",
                        obj->name);
                    /*  TROUBLESHOOT
                ...
                else if (iteration_count_val == 0)
                {
                    ...
                    if (inverter_droop_fp)
                    {
                        ...
                        else
                        {
                            ...
                            if (Pref > 3 * p_rated)
                            {  // prevents from exceeding the rated
...
void inverter::update_control_references(void)
{
    ...
    else if (four_quadrant_control_mode == FQM_CONSTANT_PQ)
    {
        ...
        {  // Requested power output (P_Out, Q_Out)
...
STATUS inverter::initalize_IEEE_1547_checks(OBJECT* parent)
{
    ...
    else
    {
        ...
        gl_warning(
            "Inverter:%d %s does not have a valid parent - 1547 checks have "
            "been disabled",
            obj->id, (obj->name ? obj->name : "Unnamed"));
        /*  TROUBLESHOOT
    ...
    *converged_failure = false;
        ...
        (checkRampRate_real || checkRampRate_reactive))
    {
        ...
        if ((phases & 0x10) == 0x10)  // Triplex
        {
            ...
            GL_THROW(
                "inverter:%d - %s - VSI mode was attempted on a "
