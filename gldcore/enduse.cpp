...
EXPORT int create_enduse_object(OBJECT *obj)
{
    // gl_output("CLIMATE_CLASS_CREATE_WRAPPER: id=%d name=%s",
    //   obj->id, obj->name ? obj->name : "(unnamed)");
    std::cerr << "create_enduse_object called" << std::endl;
    enduse *my = object_data<enduse>(obj); // or your allocation pattern
    return (my != nullptr) ? 1 : 0;
}

// C-style wrapper for the C++ init method
EXPORT int init_enduse(OBJECT *obj)
{
    ...
    try
    {
        enduse *my = object_data<enduse>(obj);
        std::cerr << "init_enduse called" << std::endl;
        return my->init(obj->parent);
    }
    catch (const char *msg)
    {
        gl_error("init_enduse_object(obj=%d; %s) error: %s", obj->id, obj->name, msg);
        return 0;
    }
}
...
enduse::enduse(MODULE *mod)
{
    ...
    if (oclass == nullptr)
    {
        ...
        oclass->init = (FUNCTIONADDR)init_enduse;
        gl_output("ENDUSE_CLASS_POINTERS: init=%p", (void *)oclass->init);
        oclass->create = (FUNCTIONADDR)create_enduse_object;
...
extern "C" TIMESTAMP enduse_syncall(TIMESTAMP t1)
{
    ...
    {
        n_threads_ed = global_threadcount;
        if (n_threads_ed > n_enduses)
            n_threads_ed = n_enduses;
        if (n_threads_ed == 0)
            n_threads_ed = 1;
