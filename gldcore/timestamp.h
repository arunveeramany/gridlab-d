...
extern "C"
{
	...
	// 3) Declarations only — no need to include globals.h here.
	GLDAPI_API TIMESTAMP convert_to_timestamp(const char *value);
	GLDAPI_API TIMESTAMP convert_to_timestamp_delta(const char *value,
													unsigned int *nanoseconds,
													double *dbl_time_value);

	// (Other timestamp API declarations that don't require globals.h)
