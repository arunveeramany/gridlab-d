...
#include "realtime.h"
#include "exec.h"
#include "kill.h" // declarations for kill_* handlers
#include "output.h"
...
#define getcwd _getcwd // map POSIX names to MSVC
...
GridLabD::GridLabD()
{
    ...
    const char *path_env = std::getenv("PATH"); // guard getenv
    global_gl_executable = findExecutable_x("gridlabd", exec_name, path_env ? std::string(path_env) : std::string());
    ...
    const char *glpath_env = std::getenv("GLPATH");
    global_gl_path = std::string((glpath_env != nullptr ? std::string(glpath_env) + env_delim : "") +
    ...
    const char *browser = std::getenv("GLBROWSER");
