...
int csv_reader::open(const char* file)
{
    ...
    if (columns_str[0] != 0)
    {
        ...
        {
            gl_error(
                "csv_reader::open ~ column header read failure from explicit "
                "headers");
            return 0;
        }
        else
    ...
    while (fgets(line, 1024, infile) != nullptr)
    {
        ...
        else
        {
            ...
            {  // read went 'backwards' or was blank, line
               // discarded.
                ;
            }
        }
...
{  // already pulled the '$' off the front
    ...
    {
        gl_error(
            "csv_reader::read_prop ~ unable to convert property \'%s\' due to "
            "type restrictions",
            propstr);
        /* TROUBLESHOOT
...
//			strcpy(guess_dt.tz, "GMT");
    ...
    if (sample_ct == 1)
    { /* only one sample ~ ignore it and keep feeding the same
